...
def simple_pipeline(x, y):
    model = make_pipeline(Tokenizer(preserve_case=False, strip_handles=False), Lemmatization(), TfidfVectorizer(lowercase=False, tokenizer=lambda _: _), RandomForestClassifier())
    model.fit(x, y)
    return model
